...
import com.me.content.feign.ContentFeign;
...
public class MyEventListener {
 ...
 /*   @InsertListenPoint
   ...
   /* @DeleteListenPoint
     ...
     * @param rowData
     */
    @ListenPoint(destination = "example", schema = "t_content", table = {"tb_content"}, eventType = {CanalEntry.EventType.UPDATE, CanalEntry.EventType.INSERT, CanalEntry.EventType.DELETE})
    public void onEventCustomUpdate(CanalEntry.EventType eventType, CanalEntry.RowData rowData) {
        //1.获取到被修改的category_id
        String categoryId = getColumnValue(eventType, rowData);

        //2.调用feign 获取数据
        Result<List< TbContent >> byCategory = contentFeign.findByCategory(Long.valueOf(categoryId));
        //3.存储到redis中
        List<TbContent> data = byCategory.getData();//List
        //4.
